...
7. **Track progress explicitly.** At the end of each response, include a structured JSON block:
...
8. **Guide toward diagnosis.** Each question should have a clear purpose – explain what the answer will tell you.
...
def get_ai_response(
    ...
    # Extract structured session update from response (if present)
    session_update = _extract_session_update(response_text)

    # Clean response for display (remove JSON block if present)
...
def _extract_session_update(text: str) -> Dict[str, Any]:
    """Extract the JSON session_update block from AI response."""
    try:
        # Look for ```json ... ``` block
        import re
        pattern = r"```json\s*(.*?)\s*```"
